...
def check_equivalence(machine1, machine2, dictionary, num_inputs):
    ...
    #THIS LINE  
    if(dictionary[machine1[0][0]] != dictionary[machine2[0][0]]):
        return 0
...
def main():
    ...
    for machine in nfa_machines:
        machine_names.append(machine[0])
    ...
    #find which machines are equivalent
    for i in range(len(dfa_machines)):
        if(machine_names[i] not in equivalent_machines_set):
            equivalent_machines_set.add(machine_names[i])
            equivalent_machines.append([machine_names[i]])
            
            for j in range(i+1, len(dfa_machines)):
                if(machine_names[j] not in equivalent_machines_set):
                    equivalence = check_equivalence(dfa_machines[i], dfa_machines[j], final_dict, num_inputs)

                    if(equivalence == 1):
                        equivalent_machines[len(equivalent_machines)-1].append(machine_names[j])
                        equivalent_machines_set.add(machine_names[j])
                          
    print(len(equivalent_machines))

    for partition in equivalent_machines:
        partition.sort()

    equivalent_machines.sort()
